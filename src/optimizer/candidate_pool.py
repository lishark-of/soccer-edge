...
from src.optimizer.correlation import correlation_discount, correlation_reason, same_match_forbidden
...
def build_candidate_pool(analysis: dict) -> list[dict]:
    ...
    for item in list(analysis.get("single_candidates", []) or []):
        ...
        market_prob = _float(item.get("fair_prob"))
    ...
    for size in range(2, max_size + 1):
        for legs in combinations(candidates, size):
            ...
            if same_match_forbidden(legs):
                results.append({"candidate_type": f"parlay_{size}x1", "rejected": True, "reject_reason": "同场互斥", "legs": legs})
                continue
